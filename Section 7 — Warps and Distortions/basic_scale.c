...
static GimpPDBStatusType basic_lens(gint32 image, gint32 drawable) {
    ...
    for(int y = 0; y < height; y++)
        for(int x = 0; x < width; x++) {
            ...
            srcX = x;
            srcY = y;
            //gegl_matrix3_transform_point(mat3, &srcX, &srcY);
            gegl_matrix3_transform_point(m, &srcX, &srcY);
            srcX *= ((double)y/height) * 2.0;
            GeglRectangle srcRect = {srcX, srcY, 1, 1};
    ...
    GimpParam       **return_vals)
{
    ...
    *return_vals  = values;
    ...
    values[0].data.d_status = basic_lens(image, drawable);
